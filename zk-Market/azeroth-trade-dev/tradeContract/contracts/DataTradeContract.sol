...
contract DataTradeContract is ZklayBase {
    ...
    // struct Order {     address orderer;     uint256 cm_pear;     uint256 cm_del;
    // } h_ct list
    ...
    // order number -> order mapping(bytes32 => Order) _order; to check trade
    ...
    event logOrder(address sender, uint256 c0, uint256 c1, uint256[] c2);
    ...
    )ZklayBase(depth, vk, price, toReceiveFee) {
    ...
    function registUserByDelegator(uint256 pk_own, uint256 pk_enc, address eoa)public payable {
        // bytes32 _addr = MiMC7._hash(bytes32(pk_own), bytes32(pk_enc));
        _registUser(_addressMap[eoa].Addr, pk_own, pk_enc, eoa);
    }

    function registUser(uint256 pk_own, uint256 pk_enc)public payable {
        // bytes32 _addr = MiMC7._hash(bytes32(pk_own), bytes32(pk_enc));
        _registUser(_addressMap[msg.sender].Addr, pk_own, pk_enc, msg.sender);
    }

    function _registUser(uint256 addr, uint256 pk_own, uint256 pk_enc, address eoa)public payable {
    ...
    function isRegisteredUser(uint256 addr)public view returns(bool) {
        return _addr_list[addr];
    }

    // function isRegisteredAzerothAddr(     uint256 addr )     public     view
    // returns (bool) {     return _addrList[addr]; }
    ...
    /*
        ...
        --------------
    */
    function registData(uint256[] memory inputs)public returns(bool) {
        // check input length
        ...
        // require( Groth16AltBN128._verify(registData_vk, proof, input_values),
        // "invalid proof");
    ...
    function isRegistered(uint256 _hct)public view returns(bool) {
    ...
    function orderData(
        ...
        require(_ENA[addr][0].sR == inputs[5], "invalid ENA(r)");
        require(_ENA[addr][0].ct == inputs[6], "invalid ENA(ct)");
        ...
        require(
            Groth16AltBN128._verify(orderData_vk, proof, input_values),
            "invalid proof"
        );

        // order number bytes32 orderNumber = MiMC7._hash(bytes32(inputs[3]),
        // bytes32(inputs[4])); _order[orderNumber].orderer = msg.sender; insert cm to
        // waitTradeList
        waitTradeList[inputs[3]] = true;
        waitTradeList[inputs[4]] = true;
        // _order[orderNumber].cm_pear = inputs[3]; _order[orderNumber].cm_del =
        // inputs[4];
        ...
        emit logOrder(msg.sender, input_values[1], input_values[2], c2);

        return true;
    }

    // function cancelOrder(     bytes32 orderNumber )     public     returns (bool)
    // {     require(_order[orderNumber].orderer == msg.sender,"Not match the
    // orderer");     require(waitTradeList[_order[orderNumber].cm_pear] == true,
    // "cm pear no exit");     require(waitTradeList[_order[orderNumber].cm_del] ==
    // true, "cm del no exit");     waitTradeList[_order[orderNumber].cm_pear] =
    // false;     waitTradeList[_order[orderNumber].cm_del] = false;     return
    // true; }
     ...
     */
    function acceptOrder(uint256[] memory proof, uint256[] memory inputs)public returns(
        bool
    ) {
        ...
        require(
            Groth16AltBN128._verify(acceptOrder_vk, proof, input_values),
            "invalid proof"
        );
    ...
    // function checkCmValidation(     bytes32 orderNumber )     public     view
    // returns (bool,bool) {     return (waitTradeList[_order[orderNumber].cm_pear],
    // waitTradeList[_order[orderNumber].cm_del]); }

    function _hash(bytes32 left, bytes32 right)internal pure override returns(
        bytes32
    ) {
        return MiMC7._hash(left, right);
    }

    function _verifyZKProof(uint256[] memory proof, uint256[] memory inputs)internal override returns(
        bool
    ) {
