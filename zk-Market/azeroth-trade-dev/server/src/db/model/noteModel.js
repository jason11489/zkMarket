...
export default class noteDB extends DBinterface {
    ...
    async INSERT_NOTE(note) {
        try {
            ...
            const ret = await this.INSERT(this.tableName, keys, values)
            return ret;
        } catch (err) {
            console.log(err);
            return false;
    ...
    async UPDATE_NOTE_toRead(note_idx_arr) {
        for (const [i, e] of note_idx_arr.entries()) {
            try {
                await this.UPDATE(this.tableName, ['is_read'], [1], ['note_idx'], [e])
            } catch (error) {
                console.log(error);
            }
        }
    ...
    async SELECT_NOTE_UNREAD_AND_UPDATE(sk_enc) {
        ...
        await this.UPDATE_NOTE_toRead(getNoteIdxArr(ret))
        return ret
    }
